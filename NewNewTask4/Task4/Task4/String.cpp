...
//Returns pointer to new String. Ensure this is deleted afterwards to prevent memory leaks.
String String::IntToString(const int& other)
{
	if (other == 0) {
		String output = "0";
		return output;
	...
	char* outputArray = new char[digits + 1];
	outputArray[digits] = '\0';
	m = other;
	digits -= 1;

	while (m > 0) {
		outputArray[digits--] = static_cast<char>((m % 10) + 48);
		m /= 10;
	}
...
String String::IntToASCII(const int& other)
{
	if (other == 0) {
		String output = "0";
		return output;
	...
	outputArray[0] = static_cast<char>(other);
